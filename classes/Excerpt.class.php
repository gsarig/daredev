...
class Excerpt {
    ...
    public static function limit($new_length = null, $more = true, $string = null, $id = null ) {
        /**
        ...
        */
        ...
        $get_excerpt = ($string) ? $string : get_the_excerpt();
        $excerpt = self::shorten($get_excerpt, $new_length);
    ...
    public static function shorten($s, $num) {
        if ($num < mb_strlen($s)) {
                $fs = mb_substr($s, 0, $num);

                for ($i = mb_strlen($fs); $i >= 0; $i--) {
                    if (mb_substr($fs, $i, 1) == ' ')
                        return mb_substr($fs, 0, $i+1).'...';
                    }
